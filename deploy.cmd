...
:: 2. Build
echo 2. Building app 
call :ExecuteCmd npm run deploy
IF !ERRORLEVEL! NEQ 0 goto error

:: 3. KuduSync dist directory files
IF /I "%IN_PLACE_DEPLOYMENT%" NEQ "1" (
  echo 3. Kudu syncing built app from dist folder to deployment target
  call :ExecuteCmd "%KUDU_SYNC_CMD%" -v 50 -f "%DEPLOYMENT_SOURCE%\build" -t "%DEPLOYMENT_TARGET%" -n "%NEXT_MANIFEST_PATH%" -p "%PREVIOUS_MANIFEST_PATH%"
  IF !ERRORLEVEL! NEQ 0 goto error
)
